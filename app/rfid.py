...
def rfid_route():
    ...
    for read in reads:
        ...
        epc = read["epc"]
        current_app.logger.info(f"Received read: Port={antenna_port}, epc={epc}")
...
def process_rfid_read(antenna_port, rfid_tag):
    ...
    if not production_line:
        logging.error("could not find production line db entry for antenna port " + antenna_port)
        return abort(500)

    # Gather the valid data to save to database
    tray = Trays.query.filter_by(rfid=rfid_tag).first()
    if not tray:
        current_app.logger.info("Invalid RFID scanned: " + rfid_tag)
        return
    if antenna.start:
